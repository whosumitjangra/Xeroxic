...
async function getDeletedAssignmentIds() {
  ...
  if (kvDeleted !== null) {
    // KV responded (even empty array) — it is the source of truth
    const arr = Array.isArray(kvDeleted) ? kvDeleted : [];
    return new Set(arr.map(id => String(id).trim().toLowerCase()));
  }
  // KV is configured but returned null (timeout/error) — do NOT fall back to
  // potentially stale local /tmp which was seeded from bundled empty data on cold start
  if (KV_URL && KV_TOKEN) {
    return new Set();
  }
  // KV not configured: local disk is the only storage
  const cfg = getStorageConfig();
...
async function getAssignments() {
  ...
  if (kvAssignments !== null) {
    // KV responded (even empty array) — it is the authoritative source
    raw = Array.isArray(kvAssignments) ? kvAssignments : [];
  } else if (KV_URL && KV_TOKEN) {
    // KV IS configured but returned null — do NOT fall back to stale seeded local /tmp.
    // The bundled data/assignments.json is intentionally empty [] to prevent re-seeding.
    raw = [];
  } else {
    // KV not configured: read local disk
    const cfg = getStorageConfig();
...
async function createAssignment({ subject, deadline, attachment, experimentNo, title, info, submissionGuidelines }) {
...
async function deleteAssignment(id) {
  ...
  // 1. Get the current raw assignment list (single read — avoid double KV round-trip)
  let raw = [];
  const kvAssignments = await kvGet('xerox_assignments');
  ...
  // 2. Record tombstone in persistent registry
  const deletedIds = await getDeletedAssignmentIds();
  deletedIds.add(cleanId);
  await saveDeletedAssignmentIds(deletedIds);

  // 3. Filter out ALL tombstoned IDs from the raw list and persist atomically
