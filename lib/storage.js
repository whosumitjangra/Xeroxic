 ...
 * [first 5 letters of student email handle] + [2-digit day of month] + [3-digit serial for that day]
...
async function generateMemorableOrderId(email, date = new Date()) {
  ...
  // Count existing orders on that specific calendar day to compute the sequential queue number
  const allOrders = await getOrders();
  const sameDayOrders = allOrders.filter(o => {
    if (!o.createdAt) return false;
    const od = new Date(o.createdAt);
    const oYmd = `${od.getFullYear()}-${String(od.getMonth() + 1).padStart(2, '0')}-${String(od.getDate()).padStart(2, '0')}`;
...
/**
 * Filter orders/requests for admin by month (YYYY-MM).
