...
const { deleteStorageFile, getSupabaseAdminClient, getBucketName } = require('./supabase');

// Maximum permitted file upload size: 50 MB
...
async function ensureMongoIndexes(db) {
  ...
  try {
    await Promise.allSettled([
      db.collection('orders').createIndex({ ownerId: 1, createdAt: -1 }),
      db.collection('orders').createIndex({ orderId: 1 }, { unique: true }),
      db.collection('orders').createIndex({ createdAt: -1 }),
      db.collection('print_requests').createIndex({ studentId: 1, createdAt: -1 }),
      db.collection('print_requests').createIndex({ orderId: 1 }),
      db.collection('print_requests').createIndex({ createdAt: -1 }),
      db.collection('users').createIndex({ email: 1 }),
      db.collection('users').createIndex({ role: 1 })
    ]);
    indexesEnsured = true;
  } catch (e) {
    console.warn('MongoDB ensureIndexes notice:', e.message);
  }
}
...
async function getMongoDb() {
  ...
  try {
    ...
    ensureMongoIndexes(cachedDb).catch(() => {});
    return cachedDb;
  } catch (err) {
...
/**
 * Helper to get year, month, and day in Indian Standard Time (IST / Asia/Kolkata).
 * Prevents Vercel UTC servers from lagging 5 hours 30 mins behind Indian college time.
 */
function getISTDateParts(date = new Date()) {
  ...
  return {
    dayStr: String(map.day || '01').padStart(2, '0'),
    monthStr: String(map.month || '01').padStart(2, '0'),
    yearStr: String(map.year || '2026')
  };
}

/**
 * Generates memorable Order IDs in the format:
 * [first 5 letters before @ or _ padded with # if <5] + [2-digit day of month in IST] + [3-digit serial for that day]
 * Examples:
 *   'sumit@...' -> 'sumit24001'
 *   'sam_123@...' -> 'sam##24001'
 *   'ali@...' -> 'ali##24001'
 *   'om_dev@...' -> 'om###24001'
 */
async function generateMemorableOrderId(email, date = new Date()) {
  const cleanEmail = String(email || 'student').trim().toLowerCase();
  // Extract handle before '@' or '_'
  const rawPrefix = cleanEmail.split(/[@_]/)[0] || '';
  const cleaned = rawPrefix.replace(/[^a-z0-9]/g, '');
  let prefix = cleaned.slice(0, 5);
  if (prefix.length < 5) {
    prefix = prefix.padEnd(5, '#');
  }

  // Strictly use Indian Standard Time (IST) for date computation
  const { dayStr, monthStr, yearStr } = getISTDateParts(date);
  const ymdPrefix = `${yearStr}-${monthStr}-${dayStr}`;

  // Count existing orders on that specific IST calendar day to compute the sequential queue number
  const allOrders = await getOrders();
  const sameDayOrders = allOrders.filter(o => {
    if (!o.createdAt) return false;
    const odParts = getISTDateParts(o.createdAt);
    const oYmd = `${odParts.yearStr}-${odParts.monthStr}-${odParts.dayStr}`;
    return oYmd === ymdPrefix;
  });

  const nextSeq = sameDayOrders.length + 1;
  const seqStr = String(nextSeq).padStart(3, '0');
  let candidateId = `${prefix}${dayStr}${seqStr}`;

  // Ensure 100% uniqueness in case of concurrent submissions
  const existingSet = new Set(allOrders.map(o => String(o.orderId || '').toLowerCase()));
  let suffixSeq = nextSeq;
  while (existingSet.has(candidateId.toLowerCase())) {
    suffixSeq++;
    candidateId = `${prefix}${dayStr}${String(suffixSeq).padStart(3, '0')}`;
  }
...
/**
 * High-performance query for student print history:
 * Limits response to previous 10 prints per student by default.
 */
async function getOrdersForUser(userId, limit = 10) {
  ...
  if (db) {
    try {
      ...
      console.warn('MongoDB getOrdersForUser error:', e.message);
    }
  }
...
/**
 * Filter orders/requests for admin by month (YYYY-MM) in Indian Standard Time (IST).
 * Enables monthly data renewal where each month starts fresh.
 */
function isDateInMonth(isoDateStr, yearMonth) {
  if (!isoDateStr) return false;
  try {
    const { yearStr, monthStr } = getISTDateParts(isoDateStr);
  ...
  if (db) {
    try {
      const result = await db.collection('orders').findOneAndUpdate(
        { orderId: { $regex: new RegExp(`^${escapeRegex(cleanId)}$`, 'i') } },
        { $set: { status: newStatus, updatedAt } },
        { returnDocument: 'after' }
      );
      if (result) {
        order = result.value || result;
        if (order._id) delete order._id;
      }
    } catch (e) {
      console.warn('MongoDB direct updateOrderStatus error:', e.message);
    }
  }

  // Fallback or local JSON file sync
  if (!order) {
    const orders = await getOrders();
    order = orders.find(o => o.orderId && o.orderId.toLowerCase() === cleanId.toLowerCase());
    if (order) {
    ...
    // Keep local cache file updated quickly without bulkWrite overhead
    const cfg = getStorageConfig();
    const local = readLocalJSON(cfg.ordersFile, []);
    const locOrd = local.find(o => o.orderId && o.orderId.toLowerCase() === cleanId.toLowerCase());
    if (locOrd) {
      locOrd.status = newStatus;
      locOrd.updatedAt = updatedAt;
      writeLocalJSON(cfg.ordersFile, local);
    }
  }
...
async function updatePrintRequestStatus(idOrOrderId, newStatus, skipOrderSync = false) {
  if (!idOrOrderId) return null;
  const cleanId = String(idOrOrderId).trim();
  const updatedAt = new Date().toISOString();
  ...
  if (db) {
    try {
      const result = await db.collection('print_requests').findOneAndUpdate(
        { $or: [
          { id: { $regex: new RegExp(`^${escapeRegex(cleanId)}$`, 'i') } },
          { orderId: { $regex: new RegExp(`^${escapeRegex(cleanId)}$`, 'i') } }
        ]},
        { $set: { requestStatus: newStatus, updatedAt } },
        { returnDocument: 'after' }
      );
      if (result) {
      ...
      console.warn('MongoDB direct updatePrintRequestStatus error:', e.message);
    }
  }
  ...
  if (!req) {
    ...
    const cfg = getStorageConfig();
    const local = readLocalJSON(cfg.printRequestsFile, []);
    const locReq = local.find(r => 
      (r.id && r.id.toLowerCase() === cleanId.toLowerCase()) || 
      (r.orderId && r.orderId.toLowerCase() === cleanId.toLowerCase())
    );
    if (locReq) {
      locReq.requestStatus = newStatus;
      locReq.updatedAt = updatedAt;
      writeLocalJSON(cfg.printRequestsFile, local);
    }
  }

  if (!req) return null;

  // Synchronize order status only if requested
  if (!skipOrderSync && req.orderId) {
    try {
  ...
  if (normSt && normSt !== 'REQUEST_RECEIVED' && normSt !== 'NEW') {
    try {
      await acknowledgeNotification(req.orderId || req.id);
    } catch (e) {}
  }
...
async function getAssignments() {
  ...
  if (expired.length > 0) {
    ...
    for (const exp of expired) {
      ...
      const atts = Array.isArray(exp.attachments) ? exp.attachments : (exp.attachment ? [exp.attachment] : []);
      for (const att of atts) {
        if (att && att.filePath) {
          deleteStorageFile(att.filePath).catch(() => {});
        }
      }
    }
...
async function createAssignment({ subject, category, deadline, attachment, attachments, experimentNo, title, info, submissionGuidelines, targetClass, batch, year, branch }) {
  ...
  } else if (attachment && (attachment.dataBase64 || attachment.filePath)) {
...
async function deleteAssignment(id) {
  ...
  // Clean up any associated Supabase storage files
  try {
    const target = raw.find(a => String(a.id || '').trim().toLowerCase() === cleanId);
    if (target) {
      const atts = Array.isArray(target.attachments) ? target.attachments : (target.attachment ? [target.attachment] : []);
      for (const att of atts) {
        if (att && att.filePath) {
          deleteStorageFile(att.filePath).catch(() => {});
        }
...
// ---------- Student User Management (Super Admin) ----------
async function getStudentUsers() {
  const users = await getUsers();
  return users
    .filter(u => (!u.role || u.role.toLowerCase() === 'student'))
    .map(u => ({
      id: u.id,
      name: u.name,
      email: u.email,
...
async function saveUploadedFileRecord(fileData) {
  const id = fileData.id || crypto.randomBytes(16).toString('hex');
  const record = {
    id,
    ownerId: fileData.ownerId,
    originalName: fileData.originalName,
    storedName: fileData.storedName || `${id}${path.extname(fileData.originalName || '')}`,
    filePath: fileData.filePath || null,
    bucket: fileData.bucket || getBucketName(),
    size: Number(fileData.size) || 0,
    mimeType: fileData.mimeType || 'application/octet-stream',
    uploadedAt: fileData.uploadedAt || new Date().toISOString(),
    storageProvider: fileData.storageProvider || 'supabase'
  };

  const filesDb = await getFiles();
  const existingIdx = filesDb.findIndex(f => f.id === id);
  if (existingIdx !== -1) {
    filesDb[existingIdx] = record;
  } else {
    filesDb.push(record);
  }
...
async function getUploadedFileBuffer(fileRecord) {
  ...
  // Supabase Storage download if filePath is present
  if (fileRecord.filePath) {
    try {
      const supabase = getSupabaseAdminClient();
      const bucket = fileRecord.bucket || getBucketName();
      const { data, error } = await supabase.storage.from(bucket).download(fileRecord.filePath);
      if (!error && data) {
        const arrayBuf = await data.arrayBuffer();
        return Buffer.from(arrayBuf);
      }
    } catch (e) {
      console.warn('[Supabase Storage] buffer download warning:', e.message);
    }
  }

  const cfg = getStorageConfig();
  if (fileRecord.ownerId && fileRecord.storedName) {
  ...
  if (fileRecord.dataBase64) {
    ...
    try {
      if (fileRecord.ownerId && fileRecord.storedName) {
        ...
        const diskPath = path.join(userUploadDir, fileRecord.storedName);
...
async function deleteUploadedFile(fileRecord) {
  ...
  // 1. Delete from Supabase Storage if filePath is present
  if (fileRecord.filePath) {
    await deleteStorageFile(fileRecord.filePath).catch(() => {});
  }
...
async function purgeStaleUnpaidOrders(maxAgeHours = 24) {
  ...
  if (db) {
    try {
      const staleOrders = await db.collection('orders').find({
      ...
      if (staleOrders.length > 0) {
        ...
        await db.collection('orders').deleteMany({ orderId: { $in: ids } });
        purgedCount = staleOrders.length;
      ...
      console.warn('MongoDB purgeStaleUnpaidOrders error:', e.message);
    }
  }

  // Also clean local JSON file
  const cfg = getStorageConfig();
  const localOrders = readLocalJSON(cfg.ordersFile, []);
  const remaining = localOrders.filter(o => {
    if (o.paymentStatus === 'PENDING_PAYMENT' && o.createdAt && o.createdAt < cutoff) {
      purgedCount++;
      return false;
    }
    return true;
  ...
  if (remaining.length !== localOrders.length) {
    writeLocalJSON(cfg.ordersFile, remaining);
  }
