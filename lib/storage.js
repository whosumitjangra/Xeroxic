...
// Primary MongoDB Atlas Configuration
const DEFAULT_MONGODB_URI = 'mongodb+srv://sumit345jangra_db_user:sumit88148@cluster0.qkxvsjs.mongodb.net/xerox?retryWrites=true&w=majority&appName=Cluster0';
...
async function updatePrintRequestStatus(idOrOrderId, newStatus) {
  ...
  if (db) {
    try {
      await db.collection('print_requests').updateOne(
        { $or: [
          { id: { $regex: new RegExp(`^${idOrOrderId}$`, 'i') } },
          { orderId: { $regex: new RegExp(`^${idOrOrderId}$`, 'i') } }
        ]},
...
async function createAssignment({ subject, category, deadline, attachment, experimentNo, title, info, submissionGuidelines, targetClass, batch, year, branch }) {
